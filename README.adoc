...
<p>
  ...
  an incomplete item (defaults to blue circle <code>🔵</code>) or a completed item (defaults to check mark <code>✅</code>). Click the icon to toggle the emoji
...
<ul>
  ...
  <li>Detects lines in <code>Agenda.txt</code> beginning with <code>🔵</code> or <code>✅</code>.</li>
  <li>Shows a yellow gutter toggle icon next to matching lines.</li>
  <li>One-click or key combination toggle between incomplete (<code>🔵</code>) and completed (<code>✅</code>).</li>
  <li>Lightweight and works with plain text.</li>
  <li>Keymap: <code>Settings | Keymap</code> → search for <code>Toggle Agenda Item</code></li>
...
<ol>
  ...
  <li>Start task lines with <code>🔵</code> for pending or <code>✅</code> for done, for example:<br/>
    <code>🔵 Draft release notes</code></li>
  <li>Operation:
      <ul>
...
<h3>Requirements</h3>
<ul>
  <li>Runs on IntelliJ Platform IDEs compatible with the plugin’s declared since-build. (2024.3+)</li>
</ul>
...
----
agenda/
├─ README.adoc                     # This file; description is injected into plugin.xml
├─ build.gradle.kts                # IntelliJ Platform Gradle Plugin build
├─ settings.gradle.kts
├─ gradle.properties               # Version and plugin display name
├─ gradle/                         # Gradle wrapper files
├─ IDE/
│  ├─ BuildPlugin.run.xml          # Run configuration: build distributable plugin ZIP
│  └─ RunIDE.run.xml               # Run configuration: launch IDE sandbox with the plugin
├─ Agenda.txt                      # Sample agenda file for quick testing
└─ src/
   ├─ main/java/none/cgutils/agenda/
   │  ├─ AgendaKeyAction.java              # Keyboard action to toggle items
   │  ├─ AgendaLineMarkerProvider.java     # Gutter icon provider
   │  ├─ AgendaToggleLogic.java            # Toggle implementation
   │  └─ settings/
   │     ├─ AgendaProjectSettings.java
   │     └─ AgendaProjectSettingsConfigurable.java
   └─ main/resources/META-INF/
      ├─ plugin.xml                # Plugin metadata; description injected from README
      └─ pluginIcon.svg
----
...
2. Go to `Settings` (on macOS `Settings` or `IntelliJ IDEA | Settings`).
3. Select `Plugins`.
4. Click the gear icon (top right) and choose `Install Plugin from Disk…`.
...
- `RunIDE.run.xml` — launches an IntelliJ sandbox with this plugin enabled.
- `BuildPlugin.run.xml` — builds the distributable plugin ZIP.

Two ways to use them:

1) Import manually
   - IntelliJ IDEA: Run > Edit Configurations… > gear icon > Import Configuration…
   - Select the `.run.xml` file from the `IDE/` directory.

2) Copy into the project’s run configuration folder
